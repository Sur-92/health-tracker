...
// ============================================
// TIME LOGS (sleep + driving; per-date entries as JSON)
// ============================================

export async function saveTimeLog(date, entries) {
  if (isElectron) {
    return window.electronAPI.saveTimeLog(date, entries);
  }

  await initWebDatabase();
  db.run('DELETE FROM time_logs WHERE date = ?', [date]);
  if (entries && entries.length > 0) {
    db.run('INSERT INTO time_logs (date, log_data) VALUES (?, ?)', [date, JSON.stringify(entries)]);
  }
  await saveToIndexedDB();
  return true;
}

export async function getTimeLog(date) {
  if (isElectron) {
    return window.electronAPI.getTimeLog(date);
  }

  await initWebDatabase();
  const results = db.exec('SELECT log_data FROM time_logs WHERE date = ?', [date]);
  if (results.length === 0 || results[0].values.length === 0) return [];
  try { return JSON.parse(results[0].values[0][0]); } catch { return []; }
}
